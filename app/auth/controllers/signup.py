...
class SignUpView(Resource):
    def post(self):
        ...
        if existing_username:
            raise HTTPException({"username": "Username is already used by someone.."})
        if existing_email:
            raise HTTPException({"email": "Email is already in use.."})

        if len(data["name"]) < 2:
            raise HTTPException(
                {"name": "First name must be greater than 1 character.."}
            )
        if data["password"] != data["password"]:
            raise ({"password": "Passwords don't match.."})
        if len(data["password"]) < 8:
            raise ({"password": "Password must be atleast 8 characters long.."})
